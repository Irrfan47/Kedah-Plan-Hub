...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($exco_user_id) {
        $stmt = $conn->prepare('SELECT p.*, u.full_name as created_by_name FROM programs p LEFT JOIN users u ON p.created_by = u.id WHERE p.created_by = ? ORDER BY p.created_at DESC');
        $stmt->bind_param('i', $exco_user_id);
        $stmt->execute();
        $result = $stmt->get_result();
    } else {
        $result = $conn->query('SELECT p.*, u.full_name as created_by_name FROM programs p LEFT JOIN users u ON p.created_by = u.id ORDER BY p.created_at DESC');
    }
    $programs = [];
